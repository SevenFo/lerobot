# copy from https://github.com/x-humanoid-robomind/x-humanoid-training-toolchain/blob/main/scripts/convert_to_lerobot.py
...
def load_config(config_path: str) -> dict:
    ...
    for field in shape_fields:
        if field in features:
            features[field]["shape"] = ast.literal_eval(
                features[field]["shape"]
            )
    ...
    try:
        with h5py.File(episode_path, "r") as file:
            ...
            camera_top_rgb_images = [
                cv2.resize(
                    cv2.imdecode(img_compressed, cv2.IMREAD_COLOR), image_size
                )
...
def main():
    ...
    parser.add_argument(
        "--repo_id", type=str, required=True, help="Dataset repository ID"
    )
    parser.add_argument(
        "--src_root", type=str, required=True, help="Source data directory"
    )
    parser.add_argument(
        "--tgt_path", type=str, required=True, help="Target output directory"
    )
    parser.add_argument(
        "--task_name",
        type=str,
